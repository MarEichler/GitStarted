...
After merging a task branch into main on GitHub, you will delete it.  This branch will no longer be available on the GitHu browser but it is still on your local machine.  

After merging and deleting the merged branch: 

1. `git fetch origin` - pull all changes to repo   
1. `git checkout main` - check out main (or develop), as long as you aren't on the branch that is going to be removed  
1. `git remote prune origin` - remove all the remote branches that have been deleted  
1. `git branch -d BRANCH` - delete branch locally, may need to use `-D` to force delete  



## Ignore any differnces on clone and resent to version on GitHub 
